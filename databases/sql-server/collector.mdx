...
Use the production [`sqlserver.yaml`](https://github.com/querycomment/collector/blob/main/config/sqlserver.yaml) after you [grant collector access](/databases/sql-server).
...
<Warning>
  For self-hosted SQL Server, run the collector on the database host when you need host CPU, memory, disk, and network metrics to describe that instance. For Azure SQL Database, host metrics describe the collector host, not the managed database service.
</Warning>
