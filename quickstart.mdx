...
Use the [`otelcol-contrib` package](https://github.com/open-telemetry/opentelemetry-collector-releases/tree/main/distributions/otelcol-contrib) with the [`sqlquery` receiver](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/receiver/sqlqueryreceiver) to collect PostgreSQL statement metrics from `pg_stat_statements`.
...
service:
  pipelines:
    metrics/postgres_statements:
      receivers: [sqlquery/pg_statements_postgres_v13_16]
```
...
| `OTLP_ENDPOINT` | QueryComment ingest endpoint, for example `https://ingest.querycomment.com`. |
| `OTLP_API_KEY` | QueryComment API ingest value from your dashboard. |
| `OTEL_SERVICE_NAME` | Service name to attach to exported metrics. |
| `OTEL_ENVIRONMENT` | Deployment environment. Defaults to `production` if omitted. |
| `CUSTOMER_ID` | Customer identifier to attach to exported metrics. |
| `OTEL_HEALTHCHECK_ENDPOINT` | Collector health check bind address, such as `0.0.0.0:13133`. |
| `OTEL_LOG_LEVEL` | Collector log level, such as `info` or `debug`. |
...
OTLP_ENDPOINT=https://ingest.querycomment.com
OTLP_API_KEY=replace-with-querycomment-api-ingest
OTEL_SERVICE_NAME=querycomment-postgres
OTEL_ENVIRONMENT=production
CUSTOMER_ID=replace-with-customer-id
OTEL_HEALTHCHECK_ENDPOINT=0.0.0.0:13133
OTEL_LOG_LEVEL=info
```
...
extensions:
  health_check:
    endpoint: ${env:OTEL_HEALTHCHECK_ENDPOINT}
...
exporters:
  otlphttp/backend:
    ...
    headers:
      authorization: ${env:OTLP_API_KEY}
    compression: gzip
    retry_on_failure:
      enabled: true
      initial_interval: 5s
