...
The collector config includes a general PostgreSQL pipeline and statement receivers for PostgreSQL 11-12, 13-16, and 17+. Choose the statement receiver that matches your PostgreSQL major version in `service.pipelines.metrics/postgres_statements.receivers`.
...
service:
  pipelines:
    metrics/postgres_general:
      receivers: [postgresql]
    metrics/postgres_statements:
      receivers: [sqlquery/pg_statements_postgres_v13_16]
```

Keep the general `postgresql` pipeline unchanged. Change only the `sqlquery/...` receiver for your PostgreSQL version.
...
receivers:
  postgresql:
    ...
    databases: [] # empty = all non-template DBs, instance-wide
    # exclude_databases: [postgres, template0, template1]
    collection_interval: 60s
    tls:
      insecure: false # TLS on. For local/dev with sslmode=disable, set true.
